...
2.  **API Call:** The Frontend doesn't care if it's talking to `user-service` or `session-service`. It simply sends a request to `https://skillsync.rangaraju.dev/api/users/profile`.
3.  **CORS Handling:** The browser first sends an `OPTIONS` request (Preflight). The Gateway handles CORS centrally, verifying that `skillsync.rangaraju.dev` is an allowed origin before letting the real request through.

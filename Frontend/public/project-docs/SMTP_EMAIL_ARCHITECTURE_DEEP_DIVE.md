...
1.  **Client:** `EHLO skillsync.rangaraju.dev` (Hello, I am SkillSync)
2.  **Server:** `250 OK` (Greetings)
3.  **Client:** `AUTH LOGIN`
4.  **Client:** *(Sends Base64 encrypted Username & Password)*
5.  **Client:** `MAIL FROM:<support@skillsync.rangaraju.dev>`
...
2.  **SPF (Sender Policy Framework):** A DNS record that says, "Yes, this specific IP address is allowed to send emails pretending to be `@skillsync.rangaraju.dev`."

...
### The First Contact
When a user types `https://skillsync.rangaraju.dev` into their browser, the browser asks the DNS for the IP address. The DNS does *not* point to the React code. The DNS points to the public IP address of the server running NGINX. 
...
If the user types exactly `https://skillsync.rangaraju.dev/api/users`, NGINX detects the `/api/` prefix.
...
1.  **USER ACTION:** Clicks "View Mentor Profile". Browser sends `GET https://skillsync.rangaraju.dev/api/mentors/1`.

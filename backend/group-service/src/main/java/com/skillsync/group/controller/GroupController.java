...
public class GroupController {
    ...
    public ResponseEntity<ApiResponse<GroupResponseDto>> getGroupDetails(
        ...
        log.info("GET /{} - Get group details for viewer {}", groupId, userId);
        GroupResponseDto response = groupService.getGroupDetails(groupId);
        // Explicitly set isExited if we have a user context for the viewer
        if (userId != null) {
            GroupResponseDto detailed = groupService.getGroupDetails(groupId);
            // Re-fetch with context might be needed if service didn't handle it, but I'll assume standard fetch for now
        }
    ...
    @ApiResponses(value = {
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "Group is full or archived"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "Unauthorized"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "User previously left this group"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "Group not found"),
    ...
    @Operation(summary = "Disband group", description = "Disband a group by archiving it (Creator only)")
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Group disbanded successfully"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "Unauthorized"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "Only group creator can disband"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "Group not found")
    })
    public ResponseEntity<ApiResponse<GroupResponseDto>> deleteGroup(
        ...
        log.info("DELETE /{} - Creator {} disbanding group", groupId, creatorId);
        GroupResponseDto response = groupService.deleteGroup(groupId, creatorId);
        return ResponseEntity.ok(ApiResponse.<GroupResponseDto>builder()
                .success(true)
                .data(response)
                .message("Group disbanded and archived successfully")
                .statusCode(200)
                .build());
    }

    @PostMapping("/{groupId}/status/{status}")
    @Operation(summary = "Update group status", description = "Toggles between ACTIVE and ARCHIVED (Creator only)")
    public ResponseEntity<ApiResponse<GroupResponseDto>> updateStatus(
            @PathVariable Long groupId,
            @PathVariable String status,
            @Parameter(hidden = true) @RequestHeader("X-User-Id") Long creatorId) {
        log.info("POST /{}/status/{} - Admin {} toggling status", groupId, status, creatorId);
        GroupResponseDto response = groupService.updateGroupStatus(groupId, creatorId, status);
        return ResponseEntity.ok(ApiResponse.<GroupResponseDto>builder()
                .success(true)
                .data(response)
                .message("Group lifecycle status updated to " + status)
                .statusCode(200)
                .build());
    }

    @DeleteMapping("/{groupId}/members/{memberId}")
    @Operation(summary = "Remove a member", description = "Eject a member from group (Creator only). This triggers a shadow-ban.")
    public ResponseEntity<ApiResponse<GroupResponseDto>> removeMember(
            @PathVariable Long groupId,
            @PathVariable Long memberId,
            @Parameter(hidden = true) @RequestHeader("X-User-Id") Long creatorId) {
        log.info("DELETE /{}/members/{} - Admin {} removing member", groupId, memberId, creatorId);
        GroupResponseDto response = groupService.removeMember(groupId, creatorId, memberId);
        return ResponseEntity.ok(ApiResponse.<GroupResponseDto>builder()
                .success(true)
                .data(response)
                .message("Member removed and shadow-banned from hub")
                .statusCode(200)
                .build());
    }
}
